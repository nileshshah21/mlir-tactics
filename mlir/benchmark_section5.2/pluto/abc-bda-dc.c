...
/* Start of CLooG code */
if ((ni >= 1) && (nj >= 1)) {
  for (t1=0;t1<=floord(nj-1,32);t1++) {
    for (t2=0;t2<=floord(nj-1,32);t2++) {
      for (t3=0;t3<=floord(ni-1,32);t3++) {
        for (t4=0;t4<=floord(ni-1,32);t4++) {
          for (t5=32*t1;t5<=min(nj-1,32*t1+31);t5++) {
            for (t6=32*t2;t6<=min(nj-1,32*t2+31);t6++) {
              for (t7=32*t4;t7<=min(ni-1,32*t4+31);t7++) {
                lbv=32*t3;
                ubv=min(ni-1,32*t3+31);
#pragma ivdep
#pragma vector always
                for (t8=lbv;t8<=ubv;t8++) {
                  C[t5][t6][t8] += A[t6][t7][t5] * B[t7][t8];;
                }
              }
            }
          }
