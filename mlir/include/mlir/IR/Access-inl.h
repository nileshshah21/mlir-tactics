...
template <typename T>
bool op_load_store_matcher<OpClass>::matchLoadOrStoreOp(T &op) {
...
bool op_load_store_matcher<OpClass>::match(Operation *op) {
  ...
  if (auto loadOp = dyn_cast<AffineLoadOp>(op))
    return matchLoadOrStoreOp<AffineLoadOp>(loadOp);
  if (auto storeOp = dyn_cast<AffineStoreOp>(op))
    return matchLoadOrStoreOp<AffineStoreOp>(storeOp);
