...
void HigherOrderPolyhedralOpt::optimizeMatmul(AffineForOp rootMatmulNest,
  ...
  LLVM_DEBUG(llvm::dbgs() << "Start vectorization\n");

  // bool clVect = false;
  if (/*clVect &&*/ jjR && !outputMemRef.getType()
                                .cast<MemRefType>()
                                .getElementType()
                                .isa<VectorType>()) {
  ...
  LLVM_DEBUG(llvm::dbgs() << "Start packing\n");

  Value lhsBuf, rhsL3Buf, rhsL1Buf;

  /// XXX: Disable CopyOp as it is broken.
  bool clCopy = false;
  // Packing.
  if (clCopy) {
    ...
    LLVM_DEBUG(llvm::dbgs() << "Start packing L2\n");
    ...
    LLVM_DEBUG(llvm::dbgs() << "End packing L2\n");
    lhsBuf = fastBuf[0];
    ...
    LLVM_DEBUG(llvm::dbgs() << "Start packing L1\n");
    ...
    cast<AllocOp>(rhsL1Buf.getDefiningOp())
        .setAttr(AllocOp::getAlignmentAttrName(),
                 builder.getI64IntegerAttr(32));
    LLVM_DEBUG(llvm::dbgs() << "End packing L1\n");
  }
  LLVM_DEBUG(llvm::dbgs() << "Start unrolling\n");
