...
public:
  ...
  PatternMatchResult matchBody(Region &body, Value i, Value j, Value k,
                               Value &operandA, Value &operandB, Value &operandC) const {
  ...
  PatternMatchResult matchAndRewriteNestedPattern(Operation *op, PatternRewriter &rewriter) const {
    ...
    auto body = [this, &operandA, &operandB, &operandC](Operation &op) -> bool {
      ...
      return matchBody(loop.getLoopBody(), i, j, k, operandA, operandB, operandC).hasValue();
    };
    ...
    SmallVector<Value, 3> values = {operandA, operandB, operandC};
    ScopedContext scop(builder, op->getLoc());
    auto matMulOp = linalg_matmul(makeValueHandles(values));
...
void RaiseAffineToLinalgPass::runOnFunction() {
  ...
  target.addLegalOp<ReturnOp, ModuleOp, ModuleTerminatorOp, FuncOp>();
