...
void ScheduleNodeMatcher::setLabel(std::string l) { label_ = l; }

std::string ScheduleNodeMatcher::getLabel() const { return label_; }
...
bool ScheduleNodeMatcher::isMatching(const ScheduleNodeMatcher &matcher,
  ...
  if (matcher.current_ == ScheduleNodeType::AnyForest) {
    if (node.has_previous_sibling()) {
      llvm_unreachable("AnyForest matcher combined with other types");
    }
