...
Match<CandidatePayload, PatternPayload>::Match(
  ...
  if (containerSize(ps) != combination.size()) {
    llvm_unreachable("expected the same number of placeholders and candidates");
  }
...
isl::map transformOneMap(
  ...
  for (const auto &rep : {arg, args...}) {
    ...
    if (!result.is_null()) {
      llvm_unreachable("one relation matched multiple patterns\n"
  ...
  for (const auto &m : matches) {
    ...
    for (const auto &candidate : toTransform) {
      ...
      if (found == originalMaps.end()) {
        llvm_unreachable(
            "could not find the matched map\n"
...
operator[](const Placeholder<CandidatePayload, PPayload> &pl) const {
  ...
  for (const auto &kvp : placeholderValues_) {
    if (kvp.first == pl.id_) {
      if (result.candidateSpaces_.empty()) {
        ...
        llvm_unreachable("different payloads for the same placeholder");
      }
      if (std::find(
  ...
  if (result.candidateSpaces_.empty()) {
    llvm_unreachable("no match for the placeholder although matches found");
  }
  return result;
}
